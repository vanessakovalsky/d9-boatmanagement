    ...
    while ($row = fgetcsv($handle, $line_max, ',')) {
      if ($send_counter != 0) {
        ...
        'type' => 'boat'
          ...
          if (!empty($row_data[5])) {
              $port = $row_data[5];
              $term = \Drupal::service('entity_type.manager')->getStorage('taxonomy_term')->loadByProperties(['name' => $port, 'vid' => 'port_d_attachement']);
              ...
              $boat->field_port_d_attachement->target_id = $port_attache;
          }
          $boat->save();
      }
      $send_counter++;
